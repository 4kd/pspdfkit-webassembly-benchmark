import { getConfigOptionsFromURL, clearAllTimings, cleanupMeasurement, median } from "./utils";
...
export function createRunner(licenseKey) {
  ...
  async function run(onChange) {
    ...
    for (let testId in tests) {
        ...
        try {
          ...
          results.forEach(result => {
            ...
            measurements[name].push(
              ...
              opts.bucket === "load"
                ? { name: result.name, duration: cleanupMeasurement(result.duration) }
      ...
      const totalTime = Math.round(medians.reduce((sum, { median }) => sum + median, 0));
