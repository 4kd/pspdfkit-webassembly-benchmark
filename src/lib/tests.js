...
export function createBenchmark(pdf, licenseKey, conf) {
  ...
  runner.bench(
    ...
    async function() {
      ...
      for (let pageIndex = 0; pageIndex < totalPageCount; pageIndex++) {
        promises.push(instance.renderPageAsArrayBuffer({ height: 1024 }, pageIndex));
      }
  ...
  runner.bench(
    ...
    async function() {
      ...
      const annotations = await Promise.all(range.map(() => instance.createAnnotation(annotation)));
  ...
  runner.bench(
    ...
    async function() {
      ...
      await prepareInstance(/* canReuseLastOne */ false, /* clearTimings */ false);
