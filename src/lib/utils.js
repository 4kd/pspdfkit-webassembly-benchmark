...
export function cleanupMeasurement(duration) {
  ...
  if (noise >= duration) {
    console.warn(
      "An error ocurred while calculating the network noise. Including network noise in this example.",
      {
        duration,
        noise,
        ignoredResources
...
export function getConfigOptionsFromURL() {
  const params = {};
  window.location.search.substring(1).replace(/([^=&]+)=([^&]*)/g, (m, key, value) => {
...
export function isWASMSupported() {
  try {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent) && /11_2_\d+/.test(navigator.userAgent)) {
      return false;
    }
  } catch (_) {
  ...
  return (
    typeof window.WebAssembly === "object" && typeof window.WebAssembly.instantiate === "function"
  );
}
